...
process.env.VIBEPLAN_API_KEY = 'test-key';

// Mock the dependencies
vi.mock('../src/vibe-client', () => {
  return {
    VibePlanClient: vi.fn().mockImplementation(() => ({
      listProjects: vi.fn(),
      getProject: vi.fn(),
      listFeatures: vi.fn(),
...
describe('MCP Tools', () => {
  ...
  vi.mocked(VibePlanClient).mockImplementation(() => stableMockClient as any);
  ...
  describe('handleCallTool', () => {
    ...
    it('streamlines project context in get_project_context', async () => {
      const fullProject = {
        ...
        analysis: { 
          features: [],
          personas: [{ id: 'p1', name: 'User', tagline: 'A user', user_role: 'admin', pain_points: {} }]
        }
      };
    ...
    it('handles sync_to_vibeplan', async () => {
      vi.mocked(walkDir).mockResolvedValue('structure-text');
    ...
    it('throws error if API key missing', async () => {
      const originalKey = process.env.VIBEPLAN_API_KEY;
      delete process.env.VIBEPLAN_API_KEY;
      ...
      await expect(handleCallTool(request as any)).rejects.toThrow('VIBEPLAN_API_KEY');
      
      process.env.VIBEPLAN_API_KEY = originalKey;
    });
    ...
    it('handles very long local path in sync_to_vibeplan', async () => {
