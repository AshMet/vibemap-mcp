    ...
    return {
        tools: [
            ...
            },
            {
                ...
                description: "Get context for a project including features, stories, personas, pages, and schema.",
                inputSchema: {
                    type: "object",
                    properties: {
...
function stripUnwantedFields(data, type) {
    ...
    for (const [key, value] of Object.entries(data)) {
        ...
        if (hasId && type === "story") {
            if (key === "persona_id" || key === "title" || key === "features")
                continue;
        }
    ...
    try {
        switch (request.params.name) {
            ...
            case "get_project_context": {
                const { projectId, includeFeatures = true, includeStories = true, includePersonas = true, includePages = true, includeSchema = true, } = request.params.arguments;
                console.error(`[DEBUG] Getting context for project: ${projectId} (Features: ${includeFeatures}, Stories: ${includeStories}, Personas: ${includePersonas}, Pages: ${includePages}, Schema: ${includeSchema})`);
                ...
                if (streamlinedProject.constraints) {
                    const { budget, business, timeline, regulatory, ...remainingConstraints } = streamlinedProject.constraints;
