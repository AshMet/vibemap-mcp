...
const FEATURE_STATUSES = ["draft", "open", "in_progress", "completed"];
const STORY_STATUSES = ["draft", "has_criteria", "open", "in_progress", "completed"];
const CRITERION_STATUSES = ["draft", "pending", "passed", "failed"];
...
const CRITERION_TRANSITIONS = {
    draft: ["pending"],
    pending: ["passed", "failed", "draft"],
    passed: ["pending"],
    failed: ["pending"],
};
function validateKanbanTransition(entityType, currentStatus, newStatus) {
    const transitions = entityType === "feature"
        ? FEATURE_TRANSITIONS
        : entityType === "story"
            ? STORY_TRANSITIONS
 ...
 */
async function guardStatusTransition(client, entityType, entityId, newStatus) {
    if (!newStatus)
        return null;
    let currentStatus;
    if (entityType === "feature") {
        const entity = (await client.getFeature(entityId));
        currentStatus = entity.status;
    }
    else if (entityType === "story") {
        const entity = (await client.getUserStory(entityId));
        currentStatus = entity.status;
    }
    else {
...
function stripFields(data, remove) {
    if (!data || typeof data !== "object")
        return data;
    if (Array.isArray(data))
        return data.map((i) => stripFields(i, remove));
    const out = {};
    ...
    return {
        tools: [
            // ── Group 1: Projects ──────────────────────────────────────────────────
            {
                ...
                description: "List all VibeMap projects for the authenticated user. Returns project IDs, names, descriptions, and status.",
                annotations: { readOnlyHint: true, destructiveHint: false },
                inputSchema: { type: "object", properties: {} },
            },
            {
                name: "vibemap_create_project",
                description: "Create a new VibeMap project. Use this when starting from an existing codebase — create the project first, then call vibemap_analyze_codebase with the returned project ID.",
                ...
                inputSchema: {
                    ...
                    properties: {
                        ...
                        description: {
                            type: "string",
                            description: "Detailed project description (50+ characters). The more detail, the better the AI analysis.",
                        },
                    },
            ...
            },
            {
                ...
                description: "Retrieve the full context of a VibeMap project including features, user stories, personas, pages, and database schema. Use this before building a feature to understand all the specs.",
                annotations: { readOnlyHint: true, destructiveHint: false },
                inputSchema: {
                    type: "object",
                    properties: {
            ...
            // ── Group 2: Features ──────────────────────────────────────────────────
            {
                ...
                description: "List features for a VibeMap project. Supports filtering by status, priority, category, and search. Returns paginated results.",
                annotations: { readOnlyHint: true, destructiveHint: false },
                inputSchema: {
                    type: "object",
                    properties: {
            ...
            },
            {
                ...
                description: "Create a new feature in a VibeMap project. Use this when reverse-engineering a codebase to register discovered capabilities.",
                annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false },
                inputSchema: {
                    type: "object",
                    properties: {
            ...
            // ── Group 3: User Stories ──────────────────────────────────────────────
            {
                ...
                description: "List user stories for a project or feature. Filter by status, priority. Returns paginated results with full story detail.",
                annotations: { readOnlyHint: true, destructiveHint: false },
                inputSchema: {
                    type: "object",
                    properties: {
            ...
            },
            {
                ...
                description: "Create a new user story inside a VibeMap feature. Provide the user role, action, and expected outcome.",
                annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false },
                inputSchema: {
                    type: "object",
                    properties: {
            ...
            // ── Group 4: Acceptance Criteria ───────────────────────────────────────
            {
                ...
                description: "List acceptance criteria for a story, feature, or project. Returns BDD-formatted criteria (Given/When/Then) with status.",
                annotations: { readOnlyHint: true, destructiveHint: false },
                inputSchema: {
                    type: "object",
                    properties: {
            ...
            },
            {
                ...
                description: "Update an acceptance criterion's status or content. Use status 'passed' when your code satisfies the criterion, 'failed' when it does not.",
                annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true },
                inputSchema: {
                    type: "object",
                    properties: {
            ...
            // ── Group 5: Kanban Tracking ───────────────────────────────────────────
            {
                ...
                description: "Atomically advance or update the kanban status of a feature, user story, or acceptance criterion. Validates allowed state transitions and prevents invalid moves. Call this when you start or finish implementing something.\n\nFeature stages: draft → open → in_progress → completed\nStory stages: draft → has_criteria → open → in_progress → completed\nCriterion stages: draft → pending → passed | failed",
                annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true },
                inputSchema: {
                    type: "object",
                    properties: {
            ...
            },
            {
                ...
                description: "Get a real-time kanban board view of a project grouped by status columns. Shows features with their stories nested underneath. Ideal for an IDE agent to understand what's planned, in progress, and done.",
                annotations: { readOnlyHint: true, destructiveHint: false },
                inputSchema: {
                    type: "object",
                    properties: {
            ...
            // ── Group 6: Codebase Analysis ─────────────────────────────────────────
            {
                ...
                description: "Scan a local directory and return a formatted tree view plus file statistics. Use this to explore and understand an existing codebase before syncing to VibeMap.",
            ...
            },
            {
                ...
                description: "Scan a local codebase and submit it to VibeMap for AI-powered reverse engineering. VibeMap will analyze the code structure and key file contents to automatically generate features, user stories, and acceptance criteria. Returns a sessionId to poll with vibemap_get_generation_status.",
            ...
            // ── Group 7: Generation Status ─────────────────────────────────────────
            {
                ...
                description: "Poll the status of a VibeMap AI generation task (e.g., reverse engineering or spec generation). Use the sessionId returned by vibemap_analyze_codebase.",
                annotations: { readOnlyHint: true, destructiveHint: false },
                inputSchema: {
                    type: "object",
                    properties: {
    ...
    try {
        switch (name) {
            ...
            case "vibemap_get_project_context": {
                ...
                const cleanProject = stripFields(project, projectFieldsToRemove);
                ...
                if (cleanProject.target_audience && typeof cleanProject.target_audience === "object") {
                    cleanProject.target_audience = {
                        primary: cleanProject.target_audience.primary,
                    };
                }
                // Strip budget/timeline from constraints
                if (cleanProject.constraints && typeof cleanProject.constraints === "object") {
                    const { budget: _b, timeline: _t, regulatory: _r, ...rest } = cleanProject.constraints;
                    cleanProject.constraints = rest;
                }
                const response = { project: cleanProject };
                ...
                if (parsed.includePersonas) {
                    ...
                    response.personas = personas.map((p) => ({
                        id: p.id,
                        name: p.name,
                ...
                if (parsed.includePages) {
                    ...
                    response.sections = sections.map((s) => ({
                        id: s.id,
                        page_id: s.page_id,
                        name: s.name,
            ...
            case "vibemap_update_kanban_status": {
                ...
                let currentStatus;
                if (entityType === "feature") {
                    const feature = (await client.getFeature(entityId));
                    currentStatus = feature.status;
                }
                else if (entityType === "story") {
                    const story = (await client.getUserStory(entityId));
                ...
                let updated;
                if (entityType === "feature") {
                    updated = await client.updateFeature(entityId, { status: newStatus });
                }
                else if (entityType === "story") {
                    updated = await client.updateUserStory(entityId, { status: newStatus });
                }
                else {
            ...
            case "vibemap_get_kanban_board": {
                ...
                const stories = storiesResult.user_stories || [];
                ...
                for (const story of stories) {
                    ...
                    if (!storyByFeature.has(fid))
                        storyByFeature.set(fid, []);
                ...
                for (const feature of features) {
                    ...
                    const featureStories = storyByFeature.get(feature.id) || [];
                    ...
                    for (const s of featureStories) {
                        const ss = s.status || "draft";
                        storyBoard[ss]?.push(stripFields(s, [...GLOBAL_STRIP, "features"]));
                    }
                    const entry = {
                        ...stripFields(feature, [...GLOBAL_STRIP, "projects"]),
                        _stories: storyBoard,
                ...
                const summary = {
                    ...
                    _totals: {
                        ...
                        featuresByStatus: Object.fromEntries(Object.entries(board).map(([k, v]) => [k, v.length])),
                    },
                };
                if (truncated) {
...
`.trim();
        ...
        if (error instanceof z.ZodError) {
            throw new McpError(ErrorCode.InvalidParams, `Invalid parameters: ${error.issues.map((issue) => `${issue.path.join(".")}: ${issue.message}`).join("; ")}`);
        }
